...
module internal ParseScript = 
  ...
  let rec private transformBlocks acc defs blocks = 
    ...
    | BlockCommand(Command "do-not-eval" _)::BlockSnippet(snip)::blocks ->
        ...
        transformBlocks acc defs blocks
    | BlockCommand(Command "slide-start" _)::blocks -> 
        let acc = EmbedParagraphs(StartSlide)::acc      
        transformBlocks acc defs blocks
    | BlockCommand(Command "slide-end" _)::blocks -> 
        let acc = EmbedParagraphs(EndSlide)::acc      
        transformBlocks acc defs blocks
    // Unknown command
    | BlockCommand(cmds)::_ ->
